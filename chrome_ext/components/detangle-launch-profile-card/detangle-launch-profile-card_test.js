...
function testComputedPropertiesCorporate() {
  var element = document.createElement('detangle-launch-profile-card');
  element.setAttribute('this-profile', detangle.Profiles.CORPORATE);
  return wait().then(function() {
    assertArrayEquals(
        [detangle.Profiles.REGULAR, detangle.Profiles.ISOLATED],
        element.childProfiles);
    assertTrue(element.hasChildProfiles);
  });
}
...
function testComputedPropertiesRegular() {
  ...
  return wait().then(function() {
    assertArrayEquals([detangle.Profiles.ISOLATED], element.childProfiles);
    assertTrue(element.hasChildProfiles);
  });
}
...
function testComputedPropertiesIsolated() {
  ...
  return wait().then(function() {
    assertArrayEquals([], element.childProfiles);
    assertFalse(element.hasChildProfiles);
  });
}
